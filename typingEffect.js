...
else{animateData[animateData.length-1][4]=parseFloat(animateData[animateData.length-1][4])*1000}
if($.parseJSON(x.getAttribute("typingKeepHeight").toLowerCase()))
{x.style.height=$(x).height()+"px"}});animateData.forEach(function(x,index)
{if(x[4]==!1)
{x[0].innerHTML="";window.i[index]=0;window.clock[index]=setInterval(function()
{if(x[5]&&document.getElementById(x[0].id).getAttribute("addTypingEffect")==null)
{clearInterval(window.clock[index]);setTimeout(function()
{if(typeof typingEffectStopped==="function")
{typingEffectStopped(x[0])}
else{console.warn("Create a function called 'typingEffectStopped(element)' to execute code once the effect has stopped.")}},10)}
...
{if($.inArray(i[index],x[3])!=-1)
{x[0].innerHTML+="<br>";i[index]+=4}
else{x[0].innerHTML+=x[1].charAt(i[index]);i[index]++}}
else{clearInterval(clock)}},x[2])}
else{loopPrintText(x,index);window.outerClock[index]=setInterval(function()
{loopPrintText(x,index)},x[4]+(x[2]*(x[1].length)))}})}
function loopPrintText(x,index)
{x[0].innerHTML="";window.i[index]=0;window.clock[index]=setInterval(function()
{if(x[5]&&document.getElementById(x[0].id).getAttribute("addTypingEffect")==null)
{clearInterval(window.clock[index]);clearInterval(window.outerClock[index]);setTimeout(function()
{if(typeof typingEffectStopped==="function")
{typingEffectStopped(x[0])}
else{console.warn("Create a function called 'typingEffectStopped(element)' to execute code once the effect has stopped.")}},10)}
else if(x[5]==!1&&document.getElementById(x[0].id).getAttribute("addTypingEffect")==null)
...
{var matchingElements=[];var allElements=document.getElementsByTagName('*');for(var i=0,n=allElements.length;i<n;i++)
{if(allElements[i].getAttribute(attribute)!==null)
{matchingElements.push(allElements[i])}}
return matchingElements}
function getIndicesOf(searchStr,str,caseSensitive)
{var searchStrLen=searchStr.length;if(searchStrLen==0){return[]}
